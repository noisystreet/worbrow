...
- **Multi-platform release artifacts**: GitHub Releases now attach `worbrow-linux-x86_64.tar.gz`, `worbrow-linux-x86_64-musl.tar.gz` (statically linked, runs on any Linux), `worbrow-macos-arm64.tar.gz`, and `worbrow-windows-x86_64.zip` (with a `SHA256SUMS` checksum) alongside the `.deb`; CI adds a cross-platform compile job mirroring the release matrix
...
- **Internationalization (L1/L2/L3)**: crates.io/GitHub metadata, the README (now bilingual with README.zh-CN.md), CLI/error/MCP copy, AGENTS.md, and the PR/issue templates are now in English; commit messages are English-only (enforced by a pre-commit hook); real-browser smoke tests run in CI; exit codes / JSON contract / `schema_version` unchanged
