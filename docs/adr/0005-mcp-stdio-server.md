...
- **得到**：MCP 客户端零配置接入；工具与 CLI 共享同一内核与输出契约（schema v1），
  行为一致；stdio 传输无端口/认证负担；默认构建即含 `worbrow mcp`
- **付出**：默认依赖树含 rmcp；可用 `--no-default-features` 去掉；MCP 模式的错误
  不再有退出码语义（由 MCP 客户端呈现 `isError`，契约 JSON 中的 `error.code` 仍可机器区分）
...
`cargo test --test mcp`：真实子进程 + stdio 管道，覆盖
