...
- **`pub(crate)` 化实现细节**：`cli`（clap 参数结构）、`drivers::{cdp, marionette,
  jsonrpc, discovery, fake}`、`engines::{bing, duckduckgo}`、`extract` 改为内部；
  保留 `drivers::resolve`/`BrowserKind`、`engines::resolve`/`AVAILABLE` 作为内部服务
  公开面（bin/tests 使用），**不进**对外稳定承诺
- **`BrowserKind` 归位**：从 `drivers` 上移为顶层配置类型；`drivers` 仅保留驱动实现
  （`resolve` 内部引用）
- **引擎扩展点**：`Config::with_provider(Box<dyn SearchProvider>)`（对齐 `with_driver`
  风格）；`app::run` 内：注入 provider 优先，未注入走 `engines::resolve` 注册表，
  二者并存
- **`Error` source chain**：`EngineFailure` 等内嵌错误补 `#[source]`，允许
  `anyhow`/自定义错误处理下钻底层原因
...
- **得到**：稳定的是"类型面"而非"模块树"——新增驱动/引擎不改公开面；外部引擎/驱动
  ...
  缺陷消除，公开类型完备可命名
- **付出**：`tests/` 与文档需同步模块可见性变化（`pub(crate)` 后集成测试改经顶层
  或 `resolve` 入口）；`lib.rs` 公开面收敛是一次可见性变更（非破坏性——均为收窄，
  不删类型）；`cli` `pub(crate)` 后 clap 依赖仍在运行时（feature 化推迟到 P2）
