...
「数据/网站」类短泛词）与结果集（标题+摘要+域名，小写）**零重叠** → 判离题 →
自动尝试下一引擎（默认引擎链改为 `bing,duckduckgo`，Bing 满足质量门禁时仍只用 Bing） |
| 改动点 | ① [app.rs](../src/app.rs) 新增 `relevant()`（纯词面统计，无 LM）；②
[domain.rs](../src/domain.rs)/[cli.rs](../src/cli.rs) `DEFAULT_ENGINE` → `"bing,duckduckgo"` |
| 契约影响 | 无新增字段/错误码；`meta.low_yield` 语义不变（单引擎离题时兜底标记 true） |
| 验证 | 真实故障样本单测（中国维基集群判离题、基金站点判相关、短泛词不命中）+ 集成
测试（离题集群触发降级 ddg、单引擎离题不崩溃、相关多词结果不降级回归） |
| 风险 | 词面重叠对同义词/缩写查询可能误判（如 "LLM" vs 全文「大语言模型」）→ 零重叠
是强信号 + 误判代价仅多试一个引擎；单词查询跳过判定（防导航性/歧义误伤） |
