...
> 读者：项目维护者 / 贡献者。状态：**P1/P2 已落地（2026-08，P1 见 [roadmap.md](roadmap.md) ✅ 标记），P3 相关性门禁已落地（2026-08），P4 枢纽页 `hub` 已落地（2026-08，ADR-012）**，随实施更新；落地决策按 AGENTS.md 记 ADR。
...
### P3：查询词重叠相关性门禁 —— ✅ 已完成（2026-08）

| 项 | 内容 |
|---|---|
| 现状 | 类型识别 + 占比覆盖词典/翻译污染；**离题但类型正常**的 Web 结果（如 Bing 对
「中国基金 数据 网站 天天基金网 蛋卷基金 净值查询」锚定首强实体「中国」，恒返回
...
不降级回归） |
| 风险 | 词面重叠对同义词/缩写查询可能误判（如 "LLM" vs 全文「大语言模型」）→ 占比
阈值 + 误判代价仅多试一个引擎；单词查询跳过判定（防导航性/歧义误伤） |

### P4：枢纽页（`result_kind=hub`）—— ✅ 已完成（2026-08，[ADR-012](adr/0012-hub-result-kind.md)）

| 项 | 内容 |
|---|---|
| 现状 | 词典/翻译/词面门禁之后，门户首页仍全部标 `web`（标题含「新闻」即 overlap 过关），DDG HTTP 一次成功、百度不上场 |
| 目标 | URL 形态标记 `hub`（空路径/locale 首页/通用频道段）；**内容型仍只计 `web`**；枢纽簇触发降级；截断前内容页优先 |
| 改动点 | ① [extract.rs](../src/extract.rs) `is_hub`；② [domain.rs](../src/domain.rs) `ResultKind::Hub`；③ [app.rs](../src/app.rs) 重排后截断 |
| 契约影响 | `result_kind` 新枚举值 `hub`（只增不改）；错误码不变 |
| 验证 | 实搜 URL 单测 + 枢纽 HTML 降级集成测试；夹具 `example.com/rust` 仍为 `web` |
| 风险 | 浅频道误标 `hub` → 回退 `web` 优先（文章 id / `.html`）；误判代价为多试引擎 |

### 已评估无效：会话预热对抗 Bing 冷会话劣化（2026-08）

假设：Bing 冷会话（无 cookie）返回劣化结果，长驻会话（MCP 会话池复用进程、
cookie 持续存在）可恢复。实测（同一 Firefox 进程内连续搜索 vs 独立新进程）：

| 查询 | 冷会话（新进程） | 热会话（同进程二次） |
|---|---|---|
| 天天基金网 蛋卷基金 净值查询 | Instagram 下载页（劣化） | 知乎乱页（劣化） |
| 中国基金 数据 … 净值查询 | 中国维基（锚定） | 中国维基（锚定） |
| Rust 异步编程 教程 | 正常 | 正常 |

结论：**预热无效**——劣化不因 cookie 累积而改善（且与先访问 bing.com 首页
预热的结果一致）。Bing 在此环境（CN 住宅 IP + headless Firefox）对中文长尾查询
的低质响应是持续稳定的，会话预热不解决；应对仍是降级链（ddg/百度兜底）。

## 4. 实施顺序

结果类型识别（`result_kind`）→ 质量降级信号 →（P2 同质化/同域名去重）→ P3 相关性门禁 → P4 枢纽页
...
5. **相关性门禁形态（P3，已落地）**：词面重叠（零重叠判离题）仍是**统计信号**而非
   "语义相关性打分"（非目标不变）——不做同义词/缩写归一，靠零重叠的强信号 + 降级链
   误判兜底；单词查询跳过判定（导航性/歧义不误伤）
